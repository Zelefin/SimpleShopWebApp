...
class Bot(BaseSettings, env_prefix="BOT_"):
    """
...
class Postgres(BaseSettings, env_prefix="POSTGRES_"):
    """
...
class Redis(BaseSettings, env_prefix="REDIS_"):
    """
...
def load_config(env_file: Optional[str] = None):
    ...
    try:
        ...
        config = Config(
            bot=Bot(_env_file=env_file),
            postgres=Postgres(_env_file=env_file),
            redis=Redis(_env_file=env_file),
